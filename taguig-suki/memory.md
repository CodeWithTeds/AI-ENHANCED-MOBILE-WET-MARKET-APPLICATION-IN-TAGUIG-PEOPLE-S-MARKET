...
- `orders` + `order_items` — customer orders (order_number `ORD-YYYYMMDD-NNNN`, status: pending→confirmed→ready→completed/cancelled — processing removed; pending→confirmed auto-advances to Ready to reduce vendor taps, payment_method: cash/gcash/maya, item snapshots with vendor_id, unit_price, subtotal)
